...
renovate_server() {
  ...
  _build "CGO_ENABLED=0 ${GOBUILD} -tags='nokube nocloud netgo ${PREDEFINED_BUILD_TAGS} ${TAGS}' ./cmd/renovate-server"
}
...
GO_LDFLAGS="-s -w \
  ...
  -X arhat.dev/renovate-server/pkg/version.arch=${ARCH} \
